...
\begin{enumerate}
    ...
    \item A composite target-similarity metric (C-Score) that integrates BERTScore (semantic), BLEU/ROUGE-L (lexical), and a precision-recall-like threshold to more fairly evaluate open-ended target generation while tolerating valid paraphrases.
...
We constructed a unified training dataset by merging several established stance detection resources, deliberately excluding all examples from the VAST benchmark (and any synthetic/ChatGPT-generated data) to enable a clean zero-shot evaluation on VAST later. The merged collection included examples from:
...
VAST was strictly excluded from training to ensure a true zero-shot test. Table~\ref{tab:test_distribution} details the exact number of examples evaluated across each split, and Table~\ref{tab:test_samples} shows representative examples from the evaluation set.
...
\subsubsection{Target Identification: C-Score}

Target quality was assessed using the C-Score, a composite metric that combines semantic and lexical similarity with a strict recall-like threshold:

\begin{equation}
\text{Similarity} = 0.6 \times \text{BERTScore}_\text{F1} + 0.2 \times \text{BLEU} + 0.2 \times \text{ROUGE-L}
\end{equation}

\begin{equation}
\text{Recall}_\text{thresh} = 
\begin{cases} 
1 & \text{if BERT}\geq 0.7 \;\land\; \text{BLEU} \geq 0.2 \;\land\; \text{ROUGE-L} \geq 0.4 \\
0 & \text{otherwise}
\end{cases}
\end{equation}

\begin{equation}
\text{C-Score} = \text{Similarity} \times \text{Recall}_\text{thresh}
\end{equation}

BERTScore uses contextual embeddings for semantic alignment, while BLEU and ROUGE-L capture lexical and structural overlap. The threshold prevents rewarding superficial matches or hallucinations. C-Score is averaged over all examples and reported separately for the two inference modes.
...
Target identification quality is measured via the C-Score composite (BERTScore + BLEU + ROUGE-L, thresholded recall). Stance classification uses Accuracy, Macro-F1, and Weighted-F1. All metrics are computed independently for each mode.
...
\hline
\textbf{Dataset} & \textbf{Avg BERTScore} & \textbf{Avg BLEU} & \textbf{Avg ROUGE-L} & \textbf{Avg Recall} & \textbf{Avg C-Score} \\
\hline
TSE Explicit     & 0.8960 & 0.1148 & 0.3175 & 0.3232 & \textbf{0.2534} \\
TSE Implicit     & \textbf{0.8881} & 0.0527 & 0.1383 & 0.1798 & 0.1385 \\
VAST Explicit    & 0.8954 & 0.0486 & 0.3103 & 0.0506 & 0.0424 \\
VAST Implicit    & 0.8739 & 0.0077 & 0.0769 & 0.0040 & 0.0033 \\
\hline
\end{tabular}
\end{table*}
...
\textbf{Dataset} & \textbf{Avg BERTScore} & \textbf{Avg BLEU} & \textbf{Avg ROUGE-L} & \textbf{Avg Recall} & \textbf{Avg C-Score} \\
\hline
TSE Explicit     & 0.8848 & \textbf{0.0962} & \textbf{0.2848} & \textbf{0.2278} & \textbf{0.1784} \\
TSE Implicit     & 0.8899 & \textbf{0.0649} & \textbf{0.1692} & \textbf{0.2232} & \textbf{0.1720} \\
VAST Explicit    & \textbf{0.8974} & 0.0534 & 0.3329 & 0.0574 & \textbf{0.0484} \\
VAST Implicit    & \textbf{0.8719} & 0.0065 & 0.0685 & 0.0025 & 0.0020 \\
\hline
\end{tabular}
\end{table*}
...
\begin{itemize}
    ...
    \item Consistent improvement in both target C-Score and stance accuracy across all splits.
...
Key observations:
\begin{itemize}
    \item \textbf{Target identification}: The analysis-conditioned branch improves C-Score on TSE Explicit (+0.075 absolute, from 0.1784 to 0.2534) and shows modest gains or stability on other splits. On VAST datasets, differences are near zero or slightly negative, likely due to the abstract nature of implicit targets where a short sentiment summary adds limited signal.
...
When these concrete targets are evaluated against the broader, abstract ground-truth labels using our strict C-Score (requiring BERTScore $\ge 0.7$, BLEU $\ge 0.2$, and ROUGE-L $\ge 0.4$), the predictions fall short of the required semantic radius, resulting in a Threshold Recall of zero. This "Abstraction Gap" explains the catastrophic drop in C-Score on the VAST dataset compared to the TSE dataset (see Section~\ref{sec:results}), suggesting that 8B-parameter models—even when specialized—still lack the deep, zero-shot abstraction capabilities required to reliably map specific conversational tokens to broad thematic labels without explicit few-shot examples.
...
Our results highlight both the potential and the stark limitations of local models when tackling zero-shot, implicit targets, particularly when subjected to the strict Threshold Recall C-Score metric. The introduction of the Linguistic Analysis step provided measurable improvements over single-pass target/stance generation. Future work will focus on further refining the stance classification head, exploring hierarchical abstraction for target generation, and investigating ways to recover from the rigid C-Score penalties using more flexible fuzzy matching techniques.
